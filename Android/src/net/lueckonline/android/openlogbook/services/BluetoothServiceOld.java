 ...
 * 
 */
public class BluetoothServiceOld extends IntentService {
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			int bondState = intent.getParcelableExtra(BluetoothDevice.EXTRA_BOND_STATE);
			int oldBondState = intent.getParcelableExtra(BluetoothDevice.EXTRA_PREVIOUS_BOND_STATE);
			
			if(bondState == BluetoothDevice.BOND_BONDED  && oldBondState != BluetoothDevice.BOND_BONDED)
				this.onNewBonding(intent);
			else
				this.onBondingReleased();
			
			
		}

		private void onBondingReleased() {
			if(repository == null)
				return;
			
		}

		private void onNewBonding(Intent intent) {
			//repository is initialized in onCreate. But in any case first check it
			if(repository == null)
				return;
			
			BluetoothDevice deviceFound = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
			
			//TODO: to fetch the devices every time may be a performance killer. Especially if many devices are present.
	...
	protected void onHandleIntent(Intent intent) {
		...
		// base implementation will stop the service
		if (btAdapter == null || distanceProvider == null)
			return;

		//We want this service to run indefinitely so we add a new intent to itself later.
		//But we don't want to respond directly to that intent.
		//so we wait for waitTime until we take the next intent!
		//we use IntentService because it executes the handleIntent method in a separate thread
		//but this probably prevents the BroadcastReceiver from working so don't wait to long
...
//		Old version. New version starts the logging in the BroadcastReceiver mReceiver
//		}
...
//			}
		...
		this.startService(new Intent(this, BluetoothServiceOld.class));
